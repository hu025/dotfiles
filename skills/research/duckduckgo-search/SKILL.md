...
- **`references/rss-fallback.md`** — When search tools fail, RSS feeds via `curl` provide a reliable fallback for industry news and articles. Covers drone industry RSS sources, curl parsing patterns, and the full "search failed → RSS recovery" workflow.
